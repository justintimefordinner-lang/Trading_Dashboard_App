...
export function StrategyTypeView({
  ...
  const open = sim && canSim ? rawOpen.map(simulatePosition) : rawOpen;
  const simToggle =
    rawOpen.length > 0 ? (
      <SimulateToggle on={sim && canSim} onToggle={() => setSim((v) => !v)} disabled={!canSim} />
    ) : undefined;
      ...
      {status === "open" ? (
        open.length === 0 ? (
          ...
          <>
            ...
            <OpenGroupCard title="Covered Calls" note="short calls against stock" items={open} variant="csp" action={simToggle} />
          </>
        ) : (
          <>
            <div className="mt-3 grid grid-cols-2 gap-2">
            ...
            <SpreadGroupCard title="Vertical Spreads" note="tap a column header to sort" spreads={sortedSpreads} emptyLabel={spreadEmpty} sort={sort} onSort={onSort} action={simToggle} />
            {orphans.length > 0 && (
